 ...
 *  2. Perhitungan sudut Math.atan2 dari titik pusat layer ke koordinat sentuh jari (RotationCalculator.touchAngle)
 *  3. Pembaruan nilai rotasi layer secara mulus bebas dari lonjakan batas ±180° (RotationCalculator.updatedRotation)
...
class RotateHandleTouchTest {
    ...
    fun `calculateTouchAngle computes exact angles with Math atan2`() {
        ...
        val angleRight = RotationCalculator.touchAngle(cx, cy, cx + 100f, cy)
        assertEquals(0f, angleRight, 0.001f)

        // Arah bawah: 90 derajat
        val angleDown = RotationCalculator.touchAngle(cx, cy, cx, cy + 100f)
        assertEquals(90f, angleDown, 0.001f)

        // Arah kiri: 180 derajat
        val angleLeft = RotationCalculator.touchAngle(cx, cy, cx - 100f, cy)
        assertEquals(180f, Math.abs(angleLeft), 0.001f)

        // Arah atas: -90 derajat
        val angleUp = RotationCalculator.touchAngle(cx, cy, cx, cy - 100f)
        assertEquals(-90f, angleUp, 0.001f)

        // Diagonal kanan-bawah: 45 derajat
        val angleDiag = RotationCalculator.touchAngle(cx, cy, cx + 50f, cy + 50f)
    ...
    fun `clockwise movement smoothly increases layer rotation`() {
        ...
        val newRotation = RotationCalculator.updatedRotation(initialLayerRotation, prevAngle, currentAngle)
    ...
    fun `rotation wraps within 0 to 360 range on underflow and overflow`() {
        ...
        val underflowRotation = RotationCalculator.updatedRotation(2f, 20f, 10f)
        assertEquals(352f, underflowRotation, 0.001f)

        // Overflow: 355 derajat + 10 derajat -> 5 derajat
        val overflowRotation = RotationCalculator.updatedRotation(355f, 10f, 20f)
